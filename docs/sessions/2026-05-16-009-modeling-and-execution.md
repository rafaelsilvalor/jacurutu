...
- **Edit 5 sweep scope — initial decision.** `Agent-kit/docs/workflows/*.md` (except `start-task.md`, handled in Edit 4) + `Agent-kit/init/*.md`. Expanded mid-flight via amendment to also include `Agent-kit/docs/prompts/*.md` after sub-edit 4f finding.
...
- **Out-of-scope items deferred to future `agent-kit-cleanup` brief.** Three items accumulated during this session (see Pending items below). Discipline: report, do not fix mid-flight. → tracked here.
...
- **Cleanup brief 010 candidate — inventory:**
  - `Agent-kit/docs/workflows/setup-code.md` Pattern 1 — "Apresenta plano numerado antes de qualquer mudança em código" contradicts the `Plan required: yes/no` flag (Pause 1 is conditional since brief 000).
  - `Agent-kit/docs/workflows/setup-code.md` Pattern 5 — "Mensagens de commit incluem trailer Co-authored-by: Claude" contradicts `CLAUDE.md` R10 and `GIT_WORKFLOW.md` G-R3.
  - `Agent-kit/docs/skills-plan/pause-3-protocol.md` lines 10 and 28 — `BRIEF_*.md` (glob form) stale refs. Escaped Edit 5's verification regex (`BRIEF_[A-Za-z0-9_-]*\.md` doesn't match `*`).
...
- **Project knowledge re-upload post-merge.** Files to refresh: `CLAUDE.md`, `Agent-kit/docs/prompts/task-brief-template.md`, `Agent-kit/docs/prompts/README.md`, `Agent-kit/docs/workflows/start-task.md`, `Agent-kit/docs/workflows/review-final-task.md`, `Agent-kit/init/07-create-brief.md`, `docs/ROADMAP.md`. Without this, the next session reads stale.
...
- **Executor output — branch `docs/bundle-ts-rules-and-workflow`:** eight commits total (six brief commits + STATE.md start + STATE.md remove pending). Files modified per executor's `git diff --stat`:
  - `Agent-kit/docs/prompts/README.md`
  - `Agent-kit/docs/prompts/task-brief-template.md`
  - `Agent-kit/docs/workflows/review-final-task.md`
  - `Agent-kit/docs/workflows/start-task.md`
  - `Agent-kit/init/07-create-brief.md`
  - `CLAUDE.md`
...
stale BRIEF_<name>.md refs em Agent-kit/; Astro Starlight no
