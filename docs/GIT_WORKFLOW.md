...
> **Note on commit-time checks (updated 2026-08-09):** the message checks that used to be the `pre-commit-self-audit` skill now run as `.claude/hooks/commit-guard.mjs`, and the architecture checks that used to be the closer's Phase A run as `.claude/hooks/architecture-guard.mjs`. Both fire on `PreToolUse` before `git commit` executes, in every session and on both shell tools — a skill could be forgotten, a hook cannot. Subject length, Conventional Commits type, imperative verb and the `Co-authored-by` trailer are denied outright; an unruled verb or a suspected secret escalates to the owner. Check 5 of the old skill (staged scope against the brief's declared edit scope) has **no successor**: its input came from the brief, which a hook cannot read. This is **complementary to** G-R8, not a substitute — `npm test` still runs via the git hook, and via the `Stop` green boundary in worktree sessions where `core.hooksPath` may be unset.
