...
- **`@jacurutu/core`** — pure domain logic and port interfaces. No I/O, clock, `fs`, or network. Holds the payload contract (`payload.ts`), the transform/policy domain (`transform.ts`, `policy.ts`), the three ports (`gateways.ts` — `JiraGateway`, `SpreadsheetGateway`, `DriveGateway`), and the export projection (`export.ts`). Never imports an adapter (R25).
- **`@jacurutu/adapter-jira`** — implements `JiraGateway` against the Jira REST API directly (`POST /rest/api/3/search/jql`, `nextPageToken` / `isLast` cursor pagination, Basic auth, raw global `fetch`). Depends on `core`.
- **`@jacurutu/adapter-drive`** — implements `DriveGateway` against Google Drive via `googleapis` + `google-auth-library` under a user OAuth Desktop loopback flow (scopes `drive.file` + `drive.metadata.readonly`; credentials in `~/.jacurutu/`, never in the repo). Five one-Drive-call primitives — resolve folder, find child, create folder, upload file, read file content — all confirmed live in brief 047; folder-tree composition and the verify-never-create policy sit above it, in the future `ship` layer. No command wires it yet. Depends on `core`.
- **`@jacurutu/adapter-sheets`** — implements `SpreadsheetGateway` against Google Sheets and Drive: the spreadsheet is a **report for the team**, a one-way projection target, never a return to state in the Sheet. It adds no OAuth scope and reuses `adapter-drive`'s authorization; `jacurutu report --profile <name>` wires it. Depends on `core` and `adapter-drive`. **Its design record, gotcha ids and open gaps live in `packages/adapter-sheets/CLAUDE.md`**, which loads when working in that package.
- **`@jacurutu/cli`** — the composition root and the only package with a `bin` (`jacurutu`). Wires adapters into `core`. Composition functions live in `run-fetch.ts` / `run-export.ts`; `cli.ts` is the entry point. Its Jira credentials come from `~/.jacurutu/jira-credentials.json`, a hand-seeded file carrying `baseUrl`, `email`, `apiToken` and the token's `expiresAt`, which retired the `JACURUTU_JIRA_BASE_URL` / `_EMAIL` / `_API_TOKEN` variables on 2026-08-20 (brief `2026-08-20-jira-credentials-file`) — there is no environment fallback, and `JACURUTU_JIRA_CREDENTIALS_FILE` overrides the path only.

The **Python laboratory lane** is the `buraqueira` repository — it does not migrate. It stays Python, and this monorepo absorbs proven mechanisms from it one at a time, as briefs (lane declared permanent 2026-08-08, `docs/explorations/python-laboratory-lane.md`; laboratory of record ruled 2026-08-14). It carries no behavior-preserving mandate — there are no production users. `sync.py` / `lib_sheets.py` were legacy reference only and the sync diff engine was never to be ported; the owner reversed that on 2026-08-14 and both files are now portable, into the Google layer, because the spreadsheet became a team report. What ports is the projection, not the mechanism: a report Jacurutu creates and shares needs create/write/share, not the cell-by-cell reconciliation that existed only because the Sheet held state.
...
The **art arm** is the product's second half: given a structured brief, Jacurutu
...
**Note on v1 freeze:** four of this namespace's exceptions — `E1`, `E2`, `E3` and `E5` — apply to the Electron-v1 codebase, currently in freeze (`MENTOR_BRIEF.md` §2). No new work resolves them; they remain documented for historical context and any critical-bug-only v1 maintenance, and they now live in `.claude/rules/v1-freeze.md` rather than here. New v2 exceptions take fresh numbering. `E6` and `E7` were claimed on 2026-08-09 and are the first in this list that are not about the v1 freeze — they are live, not legacy debt, and they are the two that remain in this file.
...
**E1, E2, E3 and E5 — the Electron-v1 freeze exceptions — live in `.claude/rules/v1-freeze.md`.** They load automatically when a v1 file (`main.js`, `psd-worker.js`, `renderer/**`, `automation/**`) is in play, which is the only moment their prohibitions apply. Moved out of this file on 2026-08-27 so the freeze's historical debt stops being read in every session. `E6` and `E7` below are live v2 exceptions and stay here.
