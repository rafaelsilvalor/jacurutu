...
- **`@jacurutu/cli`** — the composition root and the only package with a `bin` (`jacurutu`). Wires adapters into `core`. Composition functions live in `run-fetch.ts` / `run-export.ts`; `cli.ts` is the entry point.
